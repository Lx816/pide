 ...
 */
public class AllEndMeetingFragment extends BasePadFragment {
    View mRootView;
    private RelativeLayout no_w;
    private XRecyclerView all_end_listview;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        if (mRootView == null) {
            mRootView = inflater.inflate(R.layout.all_end_meeting_fragment, container, false);
            context=getActivity();
            initView();
            isInit = true;
            if (isInit && isVisible) {
    ...
    private void initView() {
        ...
        all_end_listview= (XRecyclerView) mRootView.findViewById(R.id.all_end_listview);
        //1)初始化RecyclerView设置
        all_end_listview.setPullRefreshEnabled(true);
        all_end_listview.setLoadingMoreEnabled(true);
        all_end_listview.setRefreshProgressStyle(ProgressStyle.BallSpinFadeLoader);
        all_end_listview.setLoadingMoreProgressStyle(ProgressStyle.BallClipRotate);

        //2)添加布局管理器
        LinearLayoutManager layoutManager = new LinearLayoutManager(getActivity());
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
        all_end_listview.setLayoutManager(layoutManager);
        //添加自定义的分割线
        //最后一个不显示分割线且自定义分割线
        ...
        all_end_listview.addItemDecoration(gridItemDecoration);
        //5)实现 下拉刷新和加载更多 接口
        all_end_listview.setLoadingListener(new XRecyclerView.LoadingListener() {
            @Override
        ...
        OkHttpUtils.postString().url(URLs.IMGEURL + "meet/list?pageNum=1&pageSize=10")
            ...
            public void onResponse(okhttp3.Call call, String response) {
                ...
                if(bean.getSuccess()){
                    ...
                    if(list.size()>0){
                        Util.closeLoadingDialog(context);
                        all_end_listview.setVisibility(View.VISIBLE);
                        no_w.setVisibility(View.GONE);
                        allMeetingAdapter=new AllMeetingAdapter(context,mList,"end");
                        all_end_listview.setAdapter(allMeetingAdapter);
                        ...
                        Util.closeLoadingDialog(context);
                        all_end_listview.setVisibility(View.GONE);
                        no_w.setVisibility(View.VISIBLE);
                    }
                }else {
                    Util.closeLoadingDialog(context);
                    ...
                    switch (mtype){
                        case 0:
                            ...
                            if(list.size()>0){
                                ...
                                all_end_listview.refreshComplete(); //下拉刷新完成
                            }else {
                                all_end_listview.refreshComplete(); //下拉刷新完成
                            }
                            break;
                        case 1:
                            if(list.size()>0){
                                ...
                                all_end_listview.loadMoreComplete();
                                allMeetingAdapter.notifyDataSetChanged();
                            }else {
                                all_end_listview.loadMoreComplete();
                            }
                        ...
                        case 2:
                            if(list.size()>0){
                                ...
                                allMeetingAdapter.notifyDataSetChanged();
                                all_end_listview.setNoMore(true);
                            }else {
                                all_end_listview.loadMoreComplete();
                            }
                            break;
