...
import www.pide.com.ui.activity.UnderwayDetailsActivity;

/**
 * Created by Administrator on 2020/4/1.
 * 所有会议（进行中）
 */
public class AllUnderwayMeetingFragment extends BasePadFragment {
    View mRootView;
    private RelativeLayout no_w;
    private XRecyclerView all_underway_listview;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        if (mRootView == null) {
            mRootView = inflater.inflate(R.layout.all_underway_meeting_fragment, container, false);
            context=getActivity();
            initView();
            isInit = true;
            if (isInit && isVisible) {
    ...
    private void initView() {
        ...
        all_underway_listview= (XRecyclerView) mRootView.findViewById(R.id.all_underway_listview);
        //1)初始化RecyclerView设置
        all_underway_listview.setPullRefreshEnabled(true);
        all_underway_listview.setLoadingMoreEnabled(true);
        all_underway_listview.setRefreshProgressStyle(ProgressStyle.BallSpinFadeLoader);
        all_underway_listview.setLoadingMoreProgressStyle(ProgressStyle.BallClipRotate);

        //2)添加布局管理器
        LinearLayoutManager layoutManager = new LinearLayoutManager(getActivity());
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
        all_underway_listview.setLayoutManager(layoutManager);
        //添加自定义的分割线
        ...
        all_underway_listview.addItemDecoration(divider);
        //5)实现 下拉刷新和加载更多 接口
        all_underway_listview.setLoadingListener(new XRecyclerView.LoadingListener() {
            @Override
    ...
    private void initData() {
        ...
        map.put("sign", sign);
        map.put("mStatus", "1");
        OkHttpUtils.postString().url(URLs.IMGEURL + "meet/list?pageNum=1&pageSize=10")
            ...
            public void onResponse(okhttp3.Call call, String response) {
                ...
                if(bean.getSuccess()){
                    ...
                    if(list.size()>0){
                        Util.closeLoadingDialog(context);
                        all_underway_listview.setVisibility(View.VISIBLE);
                        no_w.setVisibility(View.GONE);
                        allMeetingAdapter=new AllMeetingAdapter(context,mList,"underway");
                        all_underway_listview.setAdapter(allMeetingAdapter);
                        allMeetingAdapter.setOnItemClickListener(new AllMeetingAdapter.ClickListener() {
                            @Override
                            public void onItemClick(View v, int position) {
                                Intent intent=new Intent(context,UnderwayDetailsActivity.class);
                                intent.putExtra("meeting_code",list.get(position).getRoomNum());
                                startActivity(intent);
                            }
                        });
                    }else {
                        Util.closeLoadingDialog(context);
                        all_underway_listview.setVisibility(View.GONE);
    ...
    private void initData1(int mtype) {
        ...
        map.put("sign", sign);
        map.put("mStatus", "1");
        OkHttpUtils.postString().url(URLs.IMGEURL + "meet/list?pageNum="+times+"&pageSize=10")
                .mediaType(MediaType.parse("application/json; charset=utf-8"))
                .addHeader("jwtToken", SharedPreferencesUtil.read(context,"LtAreaPeople",URLs.TOKEN))
                .content(new Gson().toJson(map))
                .build().execute(new StringCallback() {
                    ...
                    switch (mtype){
                        case 0:
                            ...
                            if(list.size()>0){
                                ...
                                all_underway_listview.refreshComplete(); //下拉刷新完成
                            }else {
                                all_underway_listview.refreshComplete(); //下拉刷新完成
                            }
                            break;
                        case 1:
                            if(list.size()>0){
                                ...
                                all_underway_listview.loadMoreComplete();
                                allMeetingAdapter.notifyDataSetChanged();
                            }else {
                                all_underway_listview.loadMoreComplete();
                            }
                        ...
                        case 2:
                            if(list.size()>0){
                                ...
                                allMeetingAdapter.notifyDataSetChanged();
                                all_underway_listview.setNoMore(true);
                            }else {
                                all_underway_listview.loadMoreComplete();
                            }
                            break;
