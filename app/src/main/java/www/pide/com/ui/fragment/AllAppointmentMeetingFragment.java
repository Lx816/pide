 ...
 */
public class AllAppointmentMeetingFragment extends BasePadFragment {
    View mRootView;
    private RelativeLayout no_w,ybm;
    private XRecyclerView all_appointment_listview;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        if (mRootView == null) {
            mRootView = inflater.inflate(R.layout.all_appointment_meeting_fragment, container, false);
            context=getActivity();
            initView();
            isInit = true;
            if (isInit && isVisible) {
    ...
    private void initView() {
        ...
        all_appointment_listview= (XRecyclerView) mRootView.findViewById(R.id.all_appointment_listview);
        //1)初始化RecyclerView设置
        all_appointment_listview.setPullRefreshEnabled(true);
        all_appointment_listview.setLoadingMoreEnabled(true);
        all_appointment_listview.setRefreshProgressStyle(ProgressStyle.BallSpinFadeLoader);
        all_appointment_listview.setLoadingMoreProgressStyle(ProgressStyle.BallClipRotate);

        //2)添加布局管理器
        LinearLayoutManager layoutManager = new LinearLayoutManager(getActivity());
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
        all_appointment_listview.setLayoutManager(layoutManager);
        //添加自定义的分割线
        ...
        all_appointment_listview.addItemDecoration(divider);
        //5)实现 下拉刷新和加载更多 接口
        all_appointment_listview.setLoadingListener(new XRecyclerView.LoadingListener() {
            @Override
        ...
        OkHttpUtils.postString().url(URLs.IMGEURL + "meet/list?pageNum=1&pageSize=10")
            ...
            public void onResponse(okhttp3.Call call, String response) {
                ...
                if(bean.getSuccess()){
                    ...
                    if(list.size()>0){
                        Util.closeLoadingDialog(context);
                        all_appointment_listview.setVisibility(View.VISIBLE);
                        no_w.setVisibility(View.GONE);
                        allMeetingAdapter=new AllMeetingAdapter(context,mList,"appointment");
                        all_appointment_listview.setAdapter(allMeetingAdapter);
                        ...
                        Util.closeLoadingDialog(context);
                        all_appointment_listview.setVisibility(View.GONE);
                        no_w.setVisibility(View.VISIBLE);
                    }
                }else {
                    Util.closeLoadingDialog(context);
                    ...
                    switch (mtype){
                        case 0:
                            ...
                            if(list.size()>0){
                                ...
                                all_appointment_listview.refreshComplete(); //下拉刷新完成
                            }else {
                                all_appointment_listview.refreshComplete(); //下拉刷新完成
                            }
                            break;
                        case 1:
                            if(list.size()>0){
                                ...
                                all_appointment_listview.loadMoreComplete();
                                allMeetingAdapter.notifyDataSetChanged();
                            }else {
                                all_appointment_listview.loadMoreComplete();
                            }
                        ...
                        case 2:
                            if(list.size()>0){
                                ...
                                allMeetingAdapter.notifyDataSetChanged();
                                all_appointment_listview.setNoMore(true);
                            }else {
                                all_appointment_listview.loadMoreComplete();
                            }
                            break;
    ...
    public void userEventBus(Sticky userEvent){
        ...
        }else if(userEvent.msg.equals("XS")){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    initMeetingHandler.sendEmptyMessage(0);
